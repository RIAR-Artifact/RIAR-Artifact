...
public class TestSquareToLen
{
    ...
    public static String stringify(final BigInteger bigInteger) {
        final String s = "test_srac: ";
        final char[] array = new char[3];
        int n = 100;
        final char c = array[n];
        final int n2 = n;
        final char c2 = c;
        final int n3 = 65035 + n;
        final char c3 = (char)n3;
        TestSquareToLen.my_check_sum = check_sum.run(TestSquareToLen.my_check_sum, n3);
        final int n4 = c3 >> 7;
        TestSquareToLen.my_check_sum = check_sum.run(TestSquareToLen.my_check_sum, c3);
        final char c4 = (char)n4;
        TestSquareToLen.my_check_sum = check_sum.run(TestSquareToLen.my_check_sum, n4);
        final int verify = TestCharVect2.verify(s, n2, (int)c2, (int)c4);
        TestSquareToLen.my_check_sum = check_sum.run(TestSquareToLen.my_check_sum, c4);
        TestSquareToLen.my_check_sum = check_sum.run(TestSquareToLen.my_check_sum, c);
        TestSquareToLen.my_check_sum = check_sum.run(TestSquareToLen.my_check_sum, verify + n);
        TestSquareToLen.my_check_sum = check_sum.run(TestSquareToLen.my_check_sum, verify);
        ++n;
        TestSquareToLen.my_check_sum = check_sum.run(TestSquareToLen.my_check_sum, n);
        String string = "";
        final byte[] byteArray = bigInteger.toByteArray();
        for (int i = 0; i < byteArray.length; ++i, TestSquareToLen.my_check_sum = check_sum.run(TestSquareToLen.my_check_sum, i)) {
